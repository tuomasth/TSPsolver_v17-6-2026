...
/**
 * TSP Solver by Tuomas Hyvönen, Java file 6 of 7
 * 
 * 
 ...
 * 
 * F5 SOM-NN-OPT            Calculate the convex hull so its nodes (actually edge centroids) can be the input nodes and clusters, then 
 ...
 *                          Has OPT moves too. Additional nodes (clusters) can be added randomly.
 * 
 * F6 Super-OPT             F2 for a couple of times, F4 once, F5 once, choose the best and try to improve with OPT moves. 
 * 
 * 
 ...
 * @version 2.4
 */
public class TSP_Solver_UEF {
    public static boolean hamiltonian = true;
    ...
    static double maxNeighbourhoodRadius;
    static int maxSomIterations = 35;
    static double somLearningRate = 0.2; // keeps decreasing 
    static int howManyMoreTimesTheConvexHullNodes = -1;
    static String opt3preResult = null;
    ...
    /**
     * Initializing how many more additional clusters there will be.
     * For example 3 means "3 times the convex hull node count".
     * 
     * @param max int
     * @return howManyMoreTimesTheConvexHullNodes int
     */
    public static int initAdditionalClusterCount(int max) {
    // (int)(Math.random() * (max - min) + min); 16.6.2026 https://stackoverflow.com/questions/7922978/random-numbers-with-math-random-in-java
        howManyMoreTimesTheConvexHullNodes = (int)(Math.random() * (max - 1) + 1);
        return howManyMoreTimesTheConvexHullNodes;
    }
    
    /**
     * Set max Kohonen SOM iterations.
     * @param input int
     */
    public static void setMaxSOMiterations(int input) {
        maxSomIterations = input;
    }
    
    /**
     * Set Kohonen SOM learning rate.
     * @param input double
     */
    public static void setSOMlearningRate(double input) {
        somLearningRate = input;
    }
    
    /**
     * Set the Super-Opt pre result String.
     * @param input String
     */
    public static void set_SOPT_preResult(String input) {
        opt3preResult = input;
    }
    
    /**
     * Add String value to the Super-Opt pre result.
     * @param input String
     */
    public static void addTo_SOPT_preResult(String input) {
        opt3preResult += input;
    }
    
    /**
     * This method checks that the input does have correct numbers.
     * 
    ...
    public static String NearestNeighbour_Algorithm(String input) {
        ...
        if(inputresult.equals("ok") && max >=4 && EUC_2D && 
            ...
            //args[0] = "$ java -javaagent:tracker.jar TSP_Solver_UEF 500 > /dev/null";
            //ResourceTracker.premain(args[0]);
            // ^ Comment when PhD Pekka Kilpeläinen ResourceTracker.java is not used 
            // http://www.cs.uku.fi/~kilpelai/ResourceTracker/ (4/22/2017) 
            ...
            hamiltonian = checkHamiltonian(justSolutionOnly, max);
            if(hamiltonian) {
                long endTime = System.nanoTime();
                System.out.println("Time NNH (millisec): " + (endTime - startTime)/1000000);
                System.out.println("Time NNH (sec, rounded down): " + (endTime - startTime)/1000000000 + "\n");
                return "Solution: \n" + solution;
            }
            else {
     ...
     * in which order the nodes are located. 
     * 
     * @param input String
     * @return String
     */
    public static String DoubleMST_Algorithm_Prim(String input) {
        ...
        if(inputresult.equals("ok") && max >=4 && EUC_2D && 
            ...
            hamiltonian = checkHamiltonian(solution, max);
            if(hamiltonian) {
                long endTime = System.nanoTime();
                System.out.println("Time 2MST (millisec): " + (endTime - startTime)/1000000);
                System.out.println("Time 2MST (sec, rounded down): " + (endTime - startTime)/1000000000 + "\n");
     ...
     * Only inner vertices remain and need to be connected to the TSP tour. 
     ...
     * There should not really be any outer vertices though, only inner ones. 
     * 
     * @param input String
     * @return String
     */
    public static String ConvexHull_Algorithm(String input) {
        ...
        if(inputresult.equals("ok") && max >=4 && EUC_2D && 
            ...
            for(int z = 0; z < inner_v_rounds; z++) {
                ...
                //System.out.println("The next vertice: (" + 
                //        next_vertice_x + ", " + next_vertice_y + ")");
                ...
                // The next vertice has been chosen.
            ...
            if(hamiltonian) {
                ...
                System.out.println("\nTime CHH (millisec): " + (endTime - startTime)/1000000);
                System.out.println("Time CHH (sec, rounded down): " + (endTime - startTime)/1000000000);
     ...
     * Convex Hull input (hull edge centroids). Inner vertices are neurons that 
     * will be clustered. Finally, connect all the vertices with the 
     * Nearest Neighbour algorithm. Extra clusters / nodes may be added.
     * SOM neurons usually do not fully go on the inputs, only a little towards them.
     * 
     * @param input String
     * @return String
     */
    public static String SOM_NN_Algorithm(String input) {
        double tour_length = 0.0; 
        String solution = "";
        int min = 1;
        ...
        // The input is read, the algorithm starts if the input is ok.
        ...
        if(inputresult.equals("ok") && max >=4 && EUC_2D && 
            ...
            ArrayList hullPointsToApproach_pre = new ArrayList();
            for(int j = 0; j < amountOfHullVertices; j++) {
                double averageX = (edges[j][0][0] + edges[j][0][1])/2;
                double averageY = (edges[j][1][0] + edges[j][1][1])/2;
                hullPointsToApproach_pre.add(averageX);
                hullPointsToApproach_pre.add(averageY); // xy xy xy xy...
            }
            
            double maxPossibleDistanceInHull = 0;
            for(int j = 0; j < hullPointsToApproach_pre.size(); j+=2) {
                for(int k = 0; k < hullPointsToApproach_pre.size(); k+=2) {
                    double distance = Sub_algorithms.Euclidean_distance(
                                (double)hullPointsToApproach_pre.get(j), (double)hullPointsToApproach_pre.get(j+1), 
                                (double)hullPointsToApproach_pre.get(k), (double)hullPointsToApproach_pre.get(k+1));
                    if(distance > maxPossibleDistanceInHull) {
                        maxPossibleDistanceInHull = distance;
                    }
                }
            }
            maxNeighbourhoodRadius = maxPossibleDistanceInHull/1.1;
            initAdditionalClusterCount(6);
            
            // also add another set of points so there will be more clusters:
            for(int h = 0; h < howManyMoreTimesTheConvexHullNodes-1; h++) {
                for(int j = 0; j < (int)amountOfHullVertices/2; j++) {
                    int index = j + ((int)amountOfHullVertices/2);
                    double random = ((Math.random() * (5 - 2)) + 2); // ((Math.random() * (max - min)) + min)
                    double randomX = (edges[j][0][0] + edges[index][0][1])/random;
                    double randomY = (edges[j][1][0] + edges[index][1][1])/random;
                    hullPointsToApproach_pre.add(randomX+1);
                    hullPointsToApproach_pre.add(randomY+1); // still xy xy xy xy...
                }
                int index = 0;
                for(int j = (int)amountOfHullVertices/2; j < amountOfHullVertices; j++) {
                    double random = ((Math.random() * (5 - 2)) + 2);
                    double randomX = (edges[j][0][0] + edges[index][0][1])/random;
                    double randomY = (edges[j][1][0] + edges[index][1][1])/random;
                    hullPointsToApproach_pre.add(randomX+1);
                    hullPointsToApproach_pre.add(randomY+1); // still xy xy xy xy...
            ...
            double previousX = 0;
            double previousY = 0;
            boolean taken_has_false = true;
            boolean[] taken = new boolean[hullPointsToApproach_pre.size()];
            double min_distance0 = Double.MAX_VALUE;
            while(taken_has_false) {
                int index = 0;
                for(int j = 0; j < hullPointsToApproach_pre.size(); j+=2) {   // reordering
                    double distance = Sub_algorithms.Euclidean_distance_squared((double)hullPointsToApproach_pre.get(j), 
                                                                                (double)hullPointsToApproach_pre.get(j+1), 
                                                                             previousX, 
                                                                             previousY);
                    if((distance < min_distance0) && taken[j] == false) {
                ...
                taken[index] = true;
                taken[index+1] = true;
                hullPointsToApproach.add(hullPointsToApproach_pre.get(index));
                hullPointsToApproach.add(hullPointsToApproach_pre.get(index+1));
                previousX = (double)hullPointsToApproach_pre.get(index);
                previousY = (double)hullPointsToApproach_pre.get(index+1);
                taken_has_false = false;
                for(int k = 0; k < taken.length; k++) {
                    if(taken[k] == false) {
                        taken_has_false = true;
                    }
                }
                min_distance0 = Double.MAX_VALUE;
            }
            ...
            //System.out.println("Max distance in hull: " + maxPossibleDistanceInHullAvgs);
            ...
            somResultArray = KohonenSOM.performSOM(maxSomIterations, maxPossibleDistanceInHull, 
                        inputCoordinateXs, inputCoordinateYs, 
                        neuronCoordinateXs, neuronCoordinateYs, somLearningRate,
            ...
            for(int j = 0; j < amountOfHullVertices/howManyMoreTimesTheConvexHullNodes; j++) {
            ...
            while(booltable_has_false) {
                ...
                if((booltable[pointer] == false) 
                    ...
                    solutionArrayInsertIndex++;
                    addCounter++;
                    //System.out.println(addCounter + ", ADDED with pointer " + pointer + ", cluster wanted: " + clusterInTurnNow);
                    verticeadded = true;
                }
                else {
                    verticeadded = false;
            ...
            System.out.println("\nEntering the OPT phase");
            ArrayList solutionArrayList = new ArrayList();
            for(int j = 0; j < solutionArray.length; j++) {
                solutionArrayList.add(solutionArray[j]);
            }
            somSolution += "Trying to quick improve with opt moves.";
            double[][] answer = Sub_algorithms.MakeOptMoves(max, tour_length, solutionArrayList);
            ...
            if(hamiltonian) {
                long endTime = System.nanoTime();
                System.out.println("\nTime SOM-NN-OPT (millisec): " + (endTime - startTime)/1000000);
                System.out.println("Time SOM-NN-OPT (sec, rounded down): " + (endTime - startTime)/1000000000 + "\n");
     ...
     * with opt moves. NOT TO BE CONFUSED WITH A 3OPT MOVE.
     * 
     * @param text String
     * @return String
     */
    public static String OPT3_Algorithm(String text) {
        double tour_length = 0.0;
        String solution = "";
        int max = 0; // int min = 1;
        String rows[] = new String[500000];    
        ...
        if(inputresult.equals("ok") && max >=4 && EUC_2D && 
            ...
            double[][] edges = form_heurOPT(text, ERRORMSG,
                                                i, rows, max, coordinate_ids,
                                                coordinates_x, coordinates_y);
            ...
            if(hamiltonian) {
                long endTime = System.nanoTime();
                System.out.println("\nTime Super-OPT (millisec): " + (endTime - startTime)/1000000);
                System.out.println("Time Super-OPT (sec, rounded down): " + (endTime - startTime)/1000000000 + "\n");

                return "Note: if some solutions are equally good, only one will still be selected for quick opts."
                        + "\n" + opt3preResult + "\n" + 
                        "\nSolution: \n" + solution + "\nTour length: " + tour_length;
            }
            else {
    ...
    /**
     * Using 3 different heuristics for the Super OPT.
     * 
     * @param text String
     ...
     * @return edges double[][]
     */
    public static double[][] form_heurOPT(String text, String ERRORMSG,
        ...
        ArrayList som_nn_opt_coordinates = new ArrayList();
        ...
        result3 = TSP_Solver_UEF.SOM_NN_Algorithm(text);           //    call SOM-NN-OPT  (3) 
        ...
        TSP_Solver_UEF.set_SOPT_preResult("\tNNH for a couple of times:  " + tour1 + "\n\t     " + length1d + "\n" + 
                           "\tCHH for 1 time:  " + tour2 + "\n\t     " + length2d + "\n" + 
                           "\tSOM-NN-OPT for 1 time:  " + tour3 + "\n\t     " + length3d + "\n");
        ...
        for (String tourArr3a : tourArr3) {
            for (int j = 0; j < coordinate_ids.size(); j++) {
                ...
                if (value == Integer.parseInt(tourArr3a)) {
                    som_nn_opt_coordinates.add(coordinates_x.get(j));
                    som_nn_opt_coordinates.add(coordinates_y.get(j));
                }
            }
        }
        ...
        switch(bestOneOfTheTours) {
            case(1): {
                TSP_Solver_UEF.addTo_SOPT_preResult("NNH was chosen for improving purposes."
                        + " Please note that not always the solution will be improved!");
                edges = Sub_algorithms.MakeOptMoves(max, length1d, nnh_coordinates);
                break;
            }
            case(2): {
                TSP_Solver_UEF.addTo_SOPT_preResult("CHH was chosen for improving purposes."
                        + " Please note that not always the solution will be improved!");
                edges = Sub_algorithms.MakeOptMoves(max, length2d, chh_coordinates);
                break;
            }
            case(3): {
                TSP_Solver_UEF.addTo_SOPT_preResult("SOM-NN-OPT was chosen for improving purposes."
                        + " Please note that not always the solution will be improved!");
                edges = Sub_algorithms.MakeOptMoves(max, length3d, som_nn_opt_coordinates);
                break;
            }
        }
        return edges;
     ...
     * An improvement of 2-MST if made well, still uses Prim MST. Ant colony matching part included 
     ...
     * optimum solution for very large inputs. 
     * 
     * @param input String
     * @return String
     */
        ...
        if(inputresult.equals("ok") && max >=4 && EUC_2D && 
            ...
            //else {
                ...
                **/
                ...
                if(hamiltonian) {
                    long endTime = System.nanoTime();
                    System.out.println("Time CHRI (millisec): " + (endTime - startTime)/1000000);
                    System.out.println("Time CHRI (sec, rounded down): " + (endTime - startTime)/1000000000 + "\n");
